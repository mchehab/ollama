...
- `tq4k` - TurboQuant 4-bit K-only quantization, uses approximately 2/3 the memory of `f16` with near-`f16` quality. Like the other `tq*k` presets, does not require Flash Attention.
- `tq3k` - TurboQuant 3-bit K-only quantization, uses approximately 3/5 the memory of `f16` with negligible quality impact. Recommended default for memory savings.
- `tq2k` - TurboQuant 2-bit K-only quantization, uses approximately 1/2 the memory of `f16` with a small quality impact. Best when memory pressure is the binding constraint.
- `tq4` - TurboQuant 4-bit K and V quantization, uses approximately 1/4 the memory of `f16` with near-`f16` quality. Requires Flash Attention.
- `tq3` - TurboQuant 3-bit K and V quantization, uses approximately 1/5 the memory of `f16` with negligible quality impact. Requires Flash Attention.
...
<Note>
  The `tq*` (TurboQuant) cache types require a wave32 GPU (NVIDIA Pascal or newer, AMD RDNA1 or newer, or Apple Silicon) and run only through Ollama's native engine. TurboQuant applies to full-context attention layers; sliding-window layers (e.g. in Gemma) continue to use `f16` storage. Models with a learned K projection bias (e.g. the Qwen 2 family) are supported via per-block mean centring at encode time, so their K distribution is handled correctly. When the context window fills and Ollama would normally shift the conversation tail to make room, TurboQuant cache types instead reprocess the prompt in full from the new starting position — packed cells cannot be RoPE-shifted in place. This is transparent but adds a one-time prefill cost on long-running conversations.
</Note>
